...
class RentalBooks(object):
    ...
    def get_filtered_books(self, filter_setting):
        if filter_setting.type == FilterSetting.FILTER_NONE:
            ...
            filterd_books = filter(lambda book: book.is_expire_in_xdays(filter_setting.xdays), self.books)
            return RentalBooks(RentalBooks.__sort(filterd_books))
    ...
    def get_message(self, user, filter_setting, format='text'):
        message = ''
        if self.length() > 0:
            data      = {'user': user, 'books_len': self.length(), 'xdays': filter_setting.xdays}
            message += Message.create(RentalBooks.__get_template_path(filter_setting, format), data)

            date_keyed_books_dict = self.__get_date_keyed_books_dict()
            data       = {'date_keyed_books_dict': date_keyed_books_dict}
            message += Message.create(os.path.join(format, RentalBooks.TEMPLATE_BOOK_LIST), data)
        return message
